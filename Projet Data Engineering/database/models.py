...
class Product(db.Model):
    ...
    bar_code = db.Column(db.Integer, primary_key=True)
...
class Purchase(db.Model):
    ...
    bar_code = db.Column(db.String(120), nullable=False)
    price = db.Column(db.Integer, nullable=False)
    ## Auto generated timestamps for creation & update of the row
    bought_at = db.Column(db.DateTime, server_default=db.func.now())


    def __init__(self, bar_code, buyer_id, price):
        self.bar_code = bar_code
        self.buyer_id = buyer_id
        self.price = price

    def __repr__(self):
        return f"<User {self.buyer_id} bought the product number {self.bar_code} for {self.price} euros>"
'''
@dataclass
class Purchase(db.Model):
    ## For API JSON reponse
